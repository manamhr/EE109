...
#define MASKBITS (0xf0) // defined maskbit to take first 4 bits.
...
void lcd_init(void)
{
    ...
    lcd_writenibble(0b0011);    // Use lcd_writenibble to send 0b0010
...
void lcd_writenibble(unsigned char lcdbits)
{
    ...
    PORTB |=(1<<PB1); //add another one to make the delay to 250ns
